...
class UpstoxWebSocketAdapter(BaseBrokerWebSocketAdapter):
    ...
    def _extract_quote_data(
        ...
        ltp = ltpc.get("ltp", 0)
        ltq = ltpc.get("ltq", 0)
        cp  = float(ltpc.get("cp", 0) or 0)   # previous day close price from broker
        ...
        # OI and IV (present for options)
        oi = float(ff.get("oi", 0) or 0)
        iv = float(ff.get("iv", 0) or 0)
    ...
    def _extract_depth_data(self, feed_data: dict[str, Any], current_ts: int) -> dict[str, Any]:
        ...
        # Volume from daily OHLC (same location as quote mode)
        ohlc_list = market_ff.get("marketOHLC", {}).get("ohlc", [])
        ohlc = next(
            (o for o in ohlc_list if o.get("interval") == "1d"), ohlc_list[0] if ohlc_list else {}
        )
        volume = int(ohlc.get("vol", 0) or 0) if ohlc else 0

        # OI, IV, Greeks — present when broker sends fullFeed for options
        oi = float(market_ff.get("oi", 0) or 0)
        iv = float(market_ff.get("iv", 0) or 0)
        greeks = market_ff.get("optionGreeks") or {}
        delta = float(greeks.get("delta", 0) or 0)
        theta = float(greeks.get("theta", 0) or 0)
        gamma = float(greeks.get("gamma", 0) or 0)
        vega  = float(greeks.get("vega",  0) or 0)
